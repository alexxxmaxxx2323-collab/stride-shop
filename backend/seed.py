...
# Блок «О бренде» — короткая справка по каждому бренду (slug -> текст).
BRAND_DESC = {
    "nike": "Американский гигант спортивной экипировки, основан в 1964 году. "
            "Один из самых узнаваемых брендов в мире, известен инновациями в амортизации "
            "(Air, React, Zoom) и культовыми силуэтами.",
    "adidas": "Немецкий бренд с 1949 года, три полоски — символ спортивного наследия. "
              "Сочетает футбольную классику (Samba, Gazelle) и современные технологии Boost.",
    "new-balance": "Бостонская марка с 1906 года, начинала с ортопедических стелек. "
                   "Ценится за посадку, комфорт и линейку Made in USA/UK премиум-класса.",
    "puma": "Немецкий спортивный бренд с 1948 года. Узнаваемый «прыгающий кот» и баланс "
            "между спортом, ретро-силуэтами и коллаборациями с миром моды.",
    "reebok": "Британский бренд с 1958 года, ставший иконой фитнеса 80-х. "
              "Классические кожаные силуэты Classic Leather и Club C — вне времени.",
    "asics": "Японская марка с 1949 года, название — акроним «здоровый дух в здоровом теле». "
             "Известна беговыми технологиями GEL и культовой линейкой Gel-Lyte.",
    "vans": "Калифорнийский бренд скейт-культуры с 1966 года. Вулканизированная подошва "
            "и боковая полоса Side Stripe стали символами уличного стиля.",
    "converse": "Американская марка с 1908 года. Chuck Taylor All Star — одни из самых "
                "продаваемых кед в истории, вне моды и поколений.",
    "timberland": "Американский бренд с 1952 года, прославился жёлтым водонепроницаемым "
                  "ботинком. Синоним прочной обуви для города и непогоды.",
    "dr-martens": "Британская марка с 1960 года. Воздушная подушка AirWair и жёлтая прошивка "
                  "сделали ботинки символом музыкальных субкультур.",
    "palladium": "Французский бренд с 1947 года, начинал с авиационных шин. "
                 "Парусиновые ботинки в военном стиле — лёгкие и неубиваемые.",
    "clarks": "Британская обувная марка с 1825 года. Дезерты и Wallabee на креп-подошве — "
              "эталон сдержанной классики из натуральных материалов.",
}

# Блок «Состав» — характеристики (slug товара -> верх, подкладка, подошва, сезон, страна).
SPECS = {
    "nike-air-force-1":       ("Натуральная кожа", "Текстиль", "Резина", "Всесезон", "Вьетнам"),
    "nike-air-max-90":        ("Кожа, текстиль", "Текстиль", "Резина, пена Air", "Всесезон", "Вьетнам"),
    "nike-pegasus-38":        ("Текстиль, синтетика", "Текстиль", "Пена React", "Всесезон", "Индонезия"),
    "adidas-stan-smith":      ("Натуральная кожа", "Текстиль", "Резина", "Всесезон", "Индонезия"),
    "adidas-gazelle":         ("Замша", "Текстиль", "Резина", "Демисезон", "Вьетнам"),
    "adidas-samba-og":        ("Натуральная кожа, замша", "Текстиль", "Резина", "Всесезон", "Китай"),
    "nb-574":                 ("Замша, текстиль", "Текстиль", "Резина, ENCAP", "Всесезон", "Вьетнам"),
    "nb-990v6":               ("Замша, сетка", "Текстиль", "Пена FuelCell", "Всесезон", "США"),
    "puma-rs-x":              ("Текстиль, синтетика", "Текстиль", "Резина", "Демисезон", "Вьетнам"),
    "asics-gel-lyte-iii":     ("Замша, текстиль", "Текстиль", "Резина, GEL", "Демисезон", "Вьетнам"),
    "reebok-classic-leather": ("Натуральная кожа", "Текстиль", "ЭВА, резина", "Всесезон", "Вьетнам"),
    "reebok-club-c-85":       ("Натуральная кожа", "Текстиль", "Резина", "Всесезон", "Вьетнам"),
    "vans-old-skool":         ("Замша, парусина", "Текстиль", "Резина (вулканизация)", "Демисезон", "Китай"),
    "vans-authentic":         ("Парусина", "Текстиль", "Резина (вулканизация)", "Лето", "Китай"),
    "vans-sk8-hi":            ("Замша, парусина", "Текстиль", "Резина (вулканизация)", "Демисезон", "Китай"),
    "converse-chuck-taylor":  ("Парусина", "Текстиль", "Резина (вулканизация)", "Демисезон", "Вьетнам"),
    "converse-chuck-70":      ("Плотная парусина", "Текстиль", "Резина (вулканизация)", "Демисезон", "Вьетнам"),
    "puma-suede-classic":     ("Замша", "Текстиль", "Резина", "Демисезон", "Вьетнам"),
    "timberland-6-inch":      ("Нубук (водоотталкивающий)", "Текстиль", "Резина", "Зима/демисезон", "Доминикана"),
    "clarks-wallabee":        ("Нубук", "Кожа", "Креп (натур. каучук)", "Демисезон", "Вьетнам"),
    "dr-martens-1460":        ("Натуральная кожа Smooth", "Текстиль", "ПВХ AirWair", "Демисезон", "Вьетнам"),
    "dr-martens-jadon":       ("Натуральная кожа Smooth", "Текстиль", "ПВХ, платформа", "Демисезон", "Вьетнам"),
    "dr-martens-1461":        ("Натуральная кожа Smooth", "Текстиль", "ПВХ AirWair", "Всесезон", "Вьетнам"),
    "palladium-pampa-hi":     ("Парусина", "Текстиль", "Резина", "Демисезон", "Вьетнам"),
}

# Пул отзывов (автор, оценка, текст). Каждому товару достаётся детерминированный срез.
REVIEW_POOL = [
    ("Алексей М.",   5, "Размер в размер, на широкую ногу сели идеально. Качество на высоте, ношу каждый день."),
    ("Дмитрий",      5, "Брал на весну — не пожалел. Материалы приятные, швы ровные, выглядят дороже своей цены."),
    ("Марина К.",    4, "Красивые, но первые пару дней немного жёсткие. Разносились — стало комфортно."),
    ("Ирина",        5, "Заказывала мужу, в восторге. Упаковка аккуратная, доставка быстрая, всё как на фото."),
    ("Сергей П.",    5, "Беру уже вторую пару этой модели. Классика, которая не подводит."),
    ("Анна",         4, "Цвет чуть отличается от картинки, в жизни темнее. В остальном придраться не к чему."),
    ("Владимир",     5, "Отличная посадка, нога не устаёт даже после целого дня на ногах."),
    ("Екатерина С.", 5, "Лёгкие и удобные, идеально под джинсы. Рекомендую брать свой обычный размер."),
    ("Никита",       3, "Норм, но ожидал плотнее материал. За свои деньги — ок, не более."),
    ("Ольга",        5, "Ношу второй сезон, выглядят как новые. Подошва не стёрлась, очень довольна."),
    ("Павел Р.",     4, "Хорошие, единственное — узковаты в носке. Советую брать на полразмера больше."),
    ("Юлия",         5, "Это любовь! Сидят как влитые, комплименты ловлю постоянно."),
    ("Артём",        5, "Качество сборки отличное, ничего не скрипит и не отклеивается. Беру ещё."),
    ("Татьяна В.",   4, "Симпатичные и удобные, но маркие — светлые быстро пачкаются. Чищу регулярно."),
]


def reviews_for(slug: str, index: int) -> list[Review]:
    """Детерминированно выдаёт 2-4 отзыва на товар из общего пула."""
    count = 2 + (index % 3)  # 2, 3 или 4 отзыва
    picks = [REVIEW_POOL[(index * 3 + i) % len(REVIEW_POOL)] for i in range(count)]
    ...
    for i, (author, rating, text) in enumerate(picks):
        out.append(
            Review(
                ...
                rating=rating,
                text=text,
                created_at=base - timedelta(days=index + i * 5),
            )
        )
    return out
...
def main() -> None:
    with SessionLocal() as db:
        ...
        brands = {
            slug: Brand(name=name, slug=slug, description=BRAND_DESC.get(slug, ""))
        ...
        for index, row in enumerate(PRODUCTS):
            ...
            upper, lining, sole, season, country = SPECS.get(slug, ("", "", "", "", ""))
            product = Product(
                name=name,
                slug=slug,
            ...
            product.reviews.extend(reviews_for(slug, index))
            db.add(product)
